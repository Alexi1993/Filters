...
def process_initializer(app, hostname):
    ...
    logfile = get('CELERY_LOG_FILE') or None
    if logfile and '%i' in logfile.lower():
        # logfile path will differ so need to set up logging again.
        app.log.already_setup = False
    app.log.setup(int(get('CELERY_LOG_LEVEL', 0) or 0),
                  logfile,
                  bool(get('CELERY_LOG_REDIRECT', False)),
                  str(get('CELERY_LOG_REDIRECT_LEVEL')),
                  hostname=hostname)
    if get('FORKED_BY_MULTIPROCESSING'):
