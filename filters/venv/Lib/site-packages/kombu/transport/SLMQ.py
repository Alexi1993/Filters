...
class Channel(virtual.Channel):
    ...
    def slmq(self):
        if self._slmq is None:
            ...
            account = get('SLMQ_ACCOUNT', conninfo.virtual_host)
            user = get('SL_USERNAME', conninfo.userid)
            api_key = get('SL_API_KEY', conninfo.password)
            host = get('SLMQ_HOST', conninfo.hostname)
            port = get('SLMQ_PORT', conninfo.port)
            secure = bool(get(
